...
open class ImageViewModel {
    ...
    public init(imageProvider: ImageProvider, defaultImage: UIImage? = nil) {
        ...
        let compoundImageSignal = actualImageSignals.flatMap(.latest) { signal -> SignalProducer<UIImage?, NoError> in

            var transitionAction: (() -> ())? = nil

            let nonFailingSignal = signal.on { _ in
                print("on .....")
                transitionAction?()
            }
            ...
            return defaultImageSignal
                .on { _ in
                    print("default on.....")
                    transitionAction = { sink.send(value: ()) }
                }
