...
class ImageViewModelTests: XCTestCase {
    ...
    func testImageProviderParamsConsistency() {
        ...
        let downloadImages: [String: UIImage] = ["http://google.com/image.png": UIImage(),
                                                 "http://google.com/image2.png": UIImage()]
        let imageProvider = TestImageProvider(
            downloadFunc: {url,size in
                downloadedImageSize = size
                downloadURLRequested = url
                return SignalProducer(value: downloadImages[url.absoluteString]!)
        },
            resizeFunc: { image, size in
                resizedImageSizeRequested = size
                return SignalProducer(value: resizedImage)
        }
        )
        let defaultImage = UIImage()
        let imageViewModel = ImageViewModel(imageProvider: imageProvider, defaultImage: defaultImage)
        ...
        XCTAssertEqual(imageViewModel.resultImage.value, downloadImages["http://google.com/image.png"], "resized image should be equal to provided by image provider")
        ...
        let url2 = URL(string: "http://google.com/image2.png")!
        let imageSize2 = CGSize(width: 10, height: 10)

        imageViewModel.image.value = .url(url2)
        XCTAssertEqual(imageViewModel.resultImage.value, downloadImages["http://google.com/image2.png"], "resized image should be equal to provided by image provider")
